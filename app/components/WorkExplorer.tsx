...
const fireConstraints = [
  ...
  ["不能静态展示，需持续投入", "正向牵引+负向约束：火花等级给出成长目标，「待重燃」形成损失厌恶"],
    ...
    <div className="section-shell detail-body fire-case">
      ...
      <section className="fire-st-flow" aria-label="从背景到核心判断的产品推导">
        <article className="fire-stage fire-stage-s">
          ...
          <p className="t-body-sm">已有「踩踩」功能承接 QQ 资料卡点赞的轻互动习惯：低成本方式表达关注，也能外显社交活跃度。</p>
          <p className="t-body-sm">但它（数值）只能回答「我有多受欢迎」，无法回答「我和谁关系更特别」——互动完双方没有继续投入的理由。</p>
      ...
      <section className="fire-results" aria-labelledby="fire-result-title">
        ...
        <div className="fire-result-grid">
          <article className="fire-coverage-result"><div className="coverage-ring"><strong>≈70%</strong></div><p><strong>踩踩用户已进入续火花关系</strong>（暑期口径）</p><p className="fire-result-note">续火花已从子功能成长为踩踩的主链路，约七成踩踩用户完成了关系沉淀，玩法认知与参与门槛已被验证跑通。</p></article>
          <article className="fire-frequency-result"><strong>2.5×</strong><div className="comparison-bars" aria-hidden="true"><i /><i /></div><p><strong>火花用户日均互踩频次提升</strong>（日均 UV 口径）</p><p className="fire-result-note">有火花 13.0 次 / 无火花 5.2 次</p><p className="fire-result-note">天数累积与清零机制把一次性互动转化为每日习惯，火花关系是拉升踩踩活跃的核心杠杆。</p></article>
        </div>
      </section>
    </div>
  );
...
const bathSolutions = [
  ...
  [<><strong>达节点方结算</strong>——推至节点才扣除道具并增加清洁值，未达则回退且不消耗</>, "消除误扣顾虑，规则严谨且可回退"],
];
      ...
      <section className="invite-actions" aria-labelledby="invite-actions-title">
        ...
        <article className="invite-mvp">
          ...
          <div className="invite-mvp-evolution" aria-label="原方案到落地方案的取舍过程">
            ...
            <div className="invite-mvp-constraint"><small>遇到约束</small><strong>配置平台不支持该能力</strong><strong>上线窗口仅两天</strong><img className="invite-mvp-arrow" src="/arrow-right-blue.png" alt="" aria-hidden="true" /></div>
            <figure><span className="invite-figure-label">落地方案</span><img src="/invite-mvp-after.png" alt="落地方案：好友到来通知和奖励到账通知" /><figcaption className="t-body-sm">拆分为「好友到来通知 + 奖励到账通知」两条消息。方案并非最优形态，但在能力受限下<strong>守住了&quot;感知到具体是谁&quot;这一核心目标</strong>。</figcaption></figure>
          </div>
        </article>
